...
public class A_Dstream {
	...
	public static JavaDStream<String> local_stream(JavaStreamingContext jssc, String input) throws IOException {
	...
	public static void question1_spark_streaming() throws IOException {
		...
		JavaDStream<Integer> max_stream = Integer_stream.reduce((value1, value2) -> Math.max(value1, value2)); // Calculating
																												// max
																												// for
																												// each
																												// Dstream
	...
	public static void question2_spark_streaming() throws IOException {
		...
		// Calculating the max over all the data has been seen
		avg_stream.foreachRDD(x -> {
			count = count + 1;
			x.collect().stream().forEach(tuple -> {
				avg = new Tuple2<>(avg._1 + tuple._1, avg._2 + tuple._2);
				System.out.println("The avg after " + count + " windows is : " + (double) avg._1 / avg._2);
			});
		...
		try {
			...
			jssc.stop();
			System.out.println("The avg after 30 secondes of streaming is: " + (double) avg._1 / avg._2);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
